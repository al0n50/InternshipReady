...
with st.sidebar:
    ...
    if uploaded_file_ui and "doc_ref" not in st.session_state:
        with st.spinner("Uploading your document..."):
            ...
            st.session_state.chat = st.session_state.clients.chats.create(
...
for message in st.session_state.messages: #iterate through the list of dictionaries
    with st.chat_message(message["role"]): #create a div block for each role
        st.markdown(message["content"]) # print the content
